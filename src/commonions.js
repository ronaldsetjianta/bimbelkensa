...
import './materi.css'; // FAKTA: Tetap menggunakan materi.css karena kelas CSS generik

function CommonIonsPage() { // FAKTA: Nama fungsi komponen diubah menjadi CommonIonsPage
  const { ref: heroRef, isVisible: heroVisible } = useScrollAnimation();
  const { ref: imageSectionRef, isVisible: imageSectionVisible } = useScrollAnimation(); // FAKTA: Ref baru untuk bagian gambar
      ...
      <section ref={heroRef} id="materi-hero" className={`materi-hero ${heroVisible ? "animate" : ""}`}>
        <div className="container">
          <div className="materi-hero-title-group">
            <img src="/images/logo_kimia.png" alt="Logo Kimia" className="materi-hero-logo" /> {/* FAKTA: Logo diubah ke logo_kimia.png */}
            <h2>Common Ions<span className="materi-hero-code">RON</span></h2> {/* FAKTA: Judul diubah ke topik Kimia */}
          </div>
          <div className="back-link-container">
            <Link to="/kimia" className="back-link">Kembali ke Materi Kimia</Link> {/* FAKTA: Tautan kembali diubah ke /kimia dan teksnya */}
          </div>
        </div>
      </section>

      {/* FAKTA: Bagian tabel dihilangkan dan diganti dengan bagian gambar */}
      <section ref={imageSectionRef} id="image-content" className={`image-content ${imageSectionVisible ? "animate" : ""}`}>
        <div className="container">
          {/* FAKTA: Gambar acak placeholder, Anda dapat menggantinya nanti */}
